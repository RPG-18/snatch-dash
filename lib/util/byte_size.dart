...
String formatByteSize(AppLocalizations l10n, int bytes) {
  if (bytes >= _gb) {
    return '${(bytes / _gb).toStringAsFixed(1)} ${l10n.settingsMapCacheUnitGb}';
  }
  if (bytes >= _mb) {
    return '${(bytes / _mb).toStringAsFixed(1)} ${l10n.settingsMapCacheUnitMb}';
  }
  if (bytes >= _kb) {
    return '${(bytes / _kb).toStringAsFixed(0)} ${l10n.settingsMapCacheUnitKb}';
  }
