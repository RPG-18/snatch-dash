...
4. **Карточка доступа к уведомлениям медиа/звонков**: статус
   ...
   вне приложения.
5. **Карточка «Логи»**: тап по строке → `/more/logs` (экран-вьюер
...
6. **Карточка «Кеш карты»**: тайлы, которые уже показывались на карте (Route/
...
7. **Секция «Обновления»**: лейбл `l10n.settingsUpdatesTitle` над карточкой
...
8. **Карточка «О приложении»**: статичные заголовок «SnatchDash» и подзаголовок,
