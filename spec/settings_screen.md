...
Вертикальный лайаут, все карточки расположены в том же порядке, в каком перечислены.
...
2. **Карточка оффлайн-карты** тап по ней открывает экран [управления картами](./offline_maps_screen.md)
3. **Карточка выбора темы карты**. Radio group со значениями: **Светлая**, Тёмная.
   По умолчанию **Светлая**
4. **Валюта**: выпадающий список со значениями (**RUB**, INR, USD, EUR, GBP, AUD, CAD, SGD, AED),
    в каждой строке — `currencyDisplayName(l10n, c)` + `(symbol)`. Значение по умолчанию **RUB**
5. **Карточка доступа к уведомлениям медиа/звонков**: статус
   ...
   вне приложения.
6. **Карточка «Логи»**: тап по строке → `/more/logs` (экран-вьюер
...
7. **Карточка «Кеш карты»**: тайлы, которые уже показывались на карте (Route/
...
8. **Секция «Обновления»**: лейбл `l10n.settingsUpdatesTitle` над карточкой
   (тот же паттерн, что у секции «Валюта»), карточка —
   `_UpdatesCard` элементы расположены в вертикальном лайауте:
   - Текущая версия (`PackageInfo`)
   - Кнопка «Проверить обновления» выровнена по левой границе с текстом "Текущая версия". Справа спиннер, пока
     идёт проверка; работает всегда, независимо от тумблера ниже.
