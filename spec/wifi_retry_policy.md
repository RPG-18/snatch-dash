...
| `MIN_REQUEST_SPACING_MS` | 15 000 мс | `DashWifiManager` | Пол на частоту `cm.requestNetwork` — каждый вызов поднимает системный пикер, см. «Быстрый `onUnavailable`» |
| `LINK_UNAVAILABLE_EARLY_MS` | 15 000 мс | `DashWifiManager` | Ниже этого `onUnavailable` пришёл не по таймауту; половина `CONNECT_TIMEOUT` |
...
## Быстрый `onUnavailable` (замерено 24.09.2026)

`onUnavailable` у `WifiNetworkSpecifier` срабатывает **не только** по истечении
`CONNECT_TIMEOUT`. Он же отвечает, когда системный пикер не нашёл в скане сеть
с нужным именем или его закрыли — и тогда ответ приходит за секунды.

Замер на HBN-LX9, три выборки: **1.6 с, 6.0 с, 8.8 с** от `REQUESTING` до
`onUnavailable`. Ни одного случая, близкого к тридцати.

Отсюда два следствия, оба починены 24.09:

1. **Строка лога врала.** `still unavailable after 30000ms` печатала константу
   независимо от того, что произошло. Ровно тот урок, который на функцию ниже
   уже был выучен про задержку («X вычисляется сейчас — раньше печаталась
   константа») и не был перенесён на причину рядом. Теперь пишется измеренное
   время, и если оно меньше `LINK_UNAVAILABLE_EARLY_MS` — приписка, что это не
   таймаут. Префикс сохранён дословно, инвариант 11.
2. **Backoff обгонялся отказами.** Потолки `ReconnectPolicy` рассчитаны на
   попытку ценой `CONNECT_TIMEOUT`. Когда попытка стоит две секунды, те же
   потолки дают три системных диалога за двенадцать секунд — замерено
   24.09.2026, запросы в 22:01:16.7 / 22:01:17.7 / 22:01:25.2. Пауза теперь
   отсчитывается от **момента последнего запроса**, а не от момента отказа:
   `max(delayMs, MIN_REQUEST_SPACING_MS − прошло_с_запроса)`. При честном
   тридцатисекундном таймауте пол уже израсходован и ничего не меняет.

Пол живёт в `DashWifiManager`, а не в `ReconnectPolicy`: политика чистая и
про системные диалоги знать не должна.
...
4. **Закрыто 2026-09-22 — запрос переживает «Отключить».** Подтверждено на
   ...
   **Окно короче, чем кажется — замер 25.09.2026.** Придержать запрос мало:
   платформа всё равно роняет сеть, когда приложение перестаёт ей
   пользоваться. Единственное переподключение внутри окна за тот день (через
   37 с) ушло в новый `requestNetwork()`, и правильно — в логе видно, почему:

   ```
   13:55:56.968  [DashKeepAlive] Foreground service stopped — locks released
   13:55:58.411  [MapKit] onStop (foreground=false)
   13:56:32.347  [DashWifiManager] signal (last before loss): unavailable
   13:56:33.789  connect() → [wifi] REQUESTING
   ```

   `handle(Lost)` обнулил `network`, гард перехвата на нём и не сошёлся. То
   есть код отработал верно, а неверна была посылка: `LINGER_MS = 60_000`
   описывает наше намерение, но не то, сколько сеть реально живёт.

   Сколько живёт — **около 35 секунд** по одной выборке, и отсчёт, похоже,
   идёт не от «Отключить», а от момента, когда служба переднего плана отдала
   Wi-Fi-лок и приложение ушло в фон. Две версии, ни одна не проверена:
   Android сам сносит сеть `WifiNetworkSpecifier`, которой никто не
   пользуется, либо её роняет энергосбережение Wi-Fi после снятия лока.
   Различить можно, удержав лок на время окна, — но это батарея, и решать
   не рефакторингом.

   **Практический итог: перехват работает на быстрый повторный тап, а не на
   всё окно.** Все три подтверждённых случая 22.09 укладывались в 2-3 секунды.
   Раз так, `LINGER_MS` можно было бы урезать до десятков секунд без потери —
   но перед этим нужен второй замер, потому что выборка пока одна.

   **Подтверждено на железе 22.09.2026 вечером**, три цикла из трёх:
   `UserDisconnect : … ReleaseWifi(linger) …` → тап «Подключить» → `link up …
   downtime=0ms` через **8, 7 и 12 мс**, строки `[wifi] REQUESTING` нет ни в
   одном. Диалога райдер не видел.
