...
Конвейер умеет собрать формально исправный, но фактически дырявый пак: osmium
режет по кривому полигону, planetiler честно печёт тайлы из того, что ему дали,
а index.json показывает валидный sha256 файла, в котором нет половины области.
...
**Проверка ловит только полную потерю.** Порог у неё бинарный: слой либо есть,
либо нет. Частично урезанный пак её проходит — уцелевших городов хватает на весь
REQUIRED_LAYERS. В той же сборке 2026-08-31 все 84 пака отчитались OK, хотя
ru-bel содержал 9% нод области, ru-lip и ru-kgd — 14%, ru-cu — 18%. Зелёный
прогон здесь означает «катастрофы нет», а не «данные полные».
...
Проверяется:
  ...
  2. **Слои.** Набор слоёв OpenMapTiles, который есть у любого субъекта РФ
     ...
     ловит потерю городов: без застройки не бывает building/place. С 2026-09-01
     landcover и housenumber отключены глобально (defaults в planetiler.yaml),
     поэтому из проверки они выпадают сами — индикатором застройки остаются
     building и place, и проверка стала на два сигнала слабее.
