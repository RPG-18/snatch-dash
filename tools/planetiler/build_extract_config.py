...
Один конфиг на страну — как единый список «полигон -> имя выходного файла»,
который дальше читает split.py. Формат взят осмиумовский (тот же, что у
`osmium extract -c`), но целиком в -c он не передаётся: память osmium растёт
как `число_экстрактов × (макс_ид_ноды / 8)`, и на России это 120+ ГБ. split.py
идёт по списку и запускает по одному `osmium extract -p` на субъект — см.
докстринг split_subjects.
