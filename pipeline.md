...
| 6a | ✅ (2026-09-16) `dash-frame` / `dash-rtp` выделенные потоки с проверкой приоритета (4.3), плюс гейт `wakeLate=` | низкий — API кодека не тронут | строка `threads:` в ride-файле (видна уже на стенде); `wakeLate` — с заезда |
| 6b | Асинхронный `MediaCodec` (`setCallback` ДО `configure`, `start()` после `flush()`, Handler на `dash-frame`). **Отложен** до ответа гейта — см. §0.7 | средний — живой энкодер | латентность на глаз; `drainMiss` в ноль не ждать — он принадлежит кодеку |
...
**Состояние на 16.09: гейт 0 и этапы 1, 2, 4, 5 и 6a закрыты.** Осталось: этап 3
(две мелкие правки, без мотоцикла) и этап 6b, который ждёт не времени, а числа —
`wakeLate` с первого же заезда (§0.7). Гейт 0 данные дал (§0.4) и убрал из этапа 6
половину; `wakeLate` решит судьбу второй.
