    ...
    return Scaffold(
      appBar: AppBar(
        ...
        bottom: TabBar(
          controller: _tabs,
          tabs: [
            Tab(text: l10n.garageTabFuel),
            Tab(text: l10n.garageTabMaintenance),
          ],
        ),
        actions: [
          IconButton(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () => _tabs.index == 0
            ? _showAddFuel(context, ref)
            : _showAddService(context, ref),
            ...
            child: Row(
              ...
              children: [
                _Stat(
                  l10n.garageAvg30Day,
                  ui.avgKmpl30 != null
                      ? '${ui.avgKmpl30!.toStringAsFixed(1)} km/l'
                      : '—',
                ),
            ...
            children: [
              for (var i = 0; i < ui.fuel.length; i++) ...[
                ...
                ListTile(
                  ...
                  title: Text(
                    '${ui.fuel[i].fill.litres.toStringAsFixed(1)} L · ${ui.fuel[i].fill.cost.toStringAsFixed(0)}',
                  ),
                  subtitle: Text(
...
class _MaintenanceTab extends ConsumerWidget {
  ...
  IconData _iconForKey(String iconKey) =>
      _serviceIcons[iconKey] ?? _serviceIcons.values.first;
    ...
    return ListView.separated(
      ...
      itemBuilder: (context, i) {
        ...
        return Card(
          child: ListTile(
            leading: Icon(
              _iconForKey(row.item.iconKey),
              color: _toneColor(context, row.tone),
            ),
            ...
            trailing: PopupMenuButton<String>(
              onSelected: (value) {
                if (value == 'done') {
                  ref
                      .read(garageControllerProvider.notifier)
                      .markServiceDone(row.item, ui.odometerKm);
                } else if (value == 'delete') {
                  ref
                      .read(garageControllerProvider.notifier)
                      .deleteService(row.item);
                }
              },
              itemBuilder: (context) => [
                PopupMenuItem(
                  value: 'done',
                  child: Text(l10n.garageMarkDoneToday),
                ),
