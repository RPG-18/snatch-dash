...
Курганской против 37 у худшей здоровой — разрыв слишком мал для порога. (Замер
делался по сборке, где z10 ещё был; при `minzoom: 11` его считать было не по
чему, с корпусом z10-15 снова есть — но вывод «разрыв мал» от этого не меняется.)
"""
...
def check_cities(code: str, source, polygons_dir: Path) -> list[str]:
    ...
    Точки берутся с нижнего зума пака (`pack.min_zoom`, с 2026-09-09 это z10):
    выше их не больше, а тайлов вчетверо — замерено на `ru-len`, z11 даёт 1167
    тайлов и 570 НП, z12 — 4432 тайла и 532 НП. Дороги ищутся в тайле самого НП,
    а если их там нет — в восьми соседних (см. ниже, почему).

    **Смена minzoom на 10 ослабила эту проверку, и это не опечатка в
    комментарии.** Тайлов на нижнем зуме стало вчетверо меньше, а НП берутся из
    них — значит выборка беднее, и «сосед» на z10 покрывает вчетверо большую
    площадь, то есть дороги находятся легче и ложных «всё хорошо» больше. Замер
    по z11 выше показывает, что число НП с зумом почти не растёт (570 против
    532), так что вниз оно скорее падает медленно — но это ЭКСТРАПОЛЯЦИЯ, а не
    факт. Перемерить на первой полной сборке z10-15; если НП на z10 заметно
    меньше, брать точки с `min_zoom + 1`, а не с нижнего.
    ...
    **Фильтр по границе обязателен.** В пак затекают чужие города: приграничная
    ...
    на z11 видно 379 названий, из них 119 — за границей Адыгеи (на z10 доля
    чужих только выше: тайл шире). Без фильтра
