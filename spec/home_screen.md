...
3. **Сохранённые места** — Если нет сохраненных мест в базе то выводит текст "Нет сохраненных мест", если есть, то выводит горизонтальный список. Элемент списка состоит из двух вертикальных частей:  название объекта, дистанция(метры, километры). В правом верхнем уклу иконка корзины. По тапу по выбранному элемегту происходит переход в [Preview Routes Screen](./route_preview_screen.md). По
